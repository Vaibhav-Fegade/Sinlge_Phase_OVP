...
unsigned int FaultCheckDelay,ScrollTime,MessageTime,Relayontime,Delaymenu;
...
bit menumodeflag,changmenuflag;
...
#define MAX_AMP  400 //50.0A
#define MIN_AMP  100  // 10.0A
...
void R_MAIN_UserInit(void)
{
	...
	if(SetOverAmp >= 500) SetOverAmp = 100;
	if(SetDryRunAmp >= 500) SetDryRunAmp = 10;
	if(SpanVolt >= 999) SpanVolt = 323;
	if(SpanAmp >= 999) SpanAmp = 565;
	if(FinalRef >= 999) FinalRef = 512;
	if(SetOverVolt > 300) SetOverVolt = 280;
	if(SetUnderVolt > 300) SetUnderVolt = 150;
	
	StartFlag = 0;
	Fault = NO_FAULT;
	UnderAmpTime = UnderVoltTime = OverVoltTime = OverAmpTime = FAULT_CHECK_mSEC;
	
	
	AutoStartDelayTimeSec = 5000;
	Relayontime = 10000;
...
void FaultChecking(void)
{
	...
	{
		OverVoltTime = FAULT_CHECK_mSEC;
		if (DispVolt <= SetOverVolt - 15)
		{
		  Fault = NO_FAULT;
		  if (Relayontime == 0) StartFlag = 1;
		}
	}
		
	if(DispVolt && (DispVolt <= 100) && (Fault == NO_FAULT))   // Low Voltage
	{
		if(UnderVoltTime == 0)
		{
...
{ 
	...
	else
	{
		...
		else
		{
		  ...
		  {
			if(ScrollTime > 9000)// > 9sec means for 1 sec
			{
				ssd[0] = BLANK;
				ssd[1] = A;
				ssd[2] = BLANK;
			}
			else if(ScrollTime > 5000)// > 5sec means for 4 sec
			{
				Bin2Bcd(DispAmp);
				ssd[0] = digit[0];
				ssd[1] = digit[1]+DP;
			...
			else if(ScrollTime > 4000)// > 4sec means for 1 sec
			{
				ssd[0] = BLANK;
				ssd[1] = U;
				ssd[2] = BLANK;
...
{ 
	...
	else
	{
		...
		else
		{
			...
			else if((CalPara == MSG_HV || CalPara == SET_OVER_VOLT) && KeyDelay == 0)
	                {
		          CalPara = MSG_SET;
		          KeySense = 0;
			  menumodeflag = 0;
...
void UPKey(void)
{
	...
	else if(CalPara == SET_OVER_AMP)
	{
 	...
 	{
		if(++SetDryRunAmp >= (MAX_AMP - 10))//MAX_AMP-1.0A
		{
			SetDryRunAmp = (MAX_AMP - 10);
		}
	}
}
void DownKey(void)
{
	...
	{
		if(SetOverAmp > 100)SetOverAmp--;
	}
	else if(CalPara == SET_DRYRUN_AMP)
 	{
		if(SetDryRunAmp)SetDryRunAmp--;
	}
}
void KeyCheck(void)
{
	...
	else if(!UP_KEY)
	{
		...
		else
		{
			...
			{
			        if (CalPara != MSG_OL && CalPara != MSG_HV && CalPara != MSG_SET)
				{
				if(StartFlag)
					StartFlag = 0;//OFF;
				}
	...
	else 
	{
		...
		if(CalPara > CAL_30A)
		{
			if(++AutoOut >= 100)//100 * 100 = 10000 = 10 sec
			{
				AutoOut = 0;
				CalPara = 0;
				Write_Memory();
...
void FDL_Read(void)
{
	...
	FDL_Request.data_pu08=Read_DataBuffer; 			//Assign the address of Read_DataBuffer to stored contents of  dedicated Data Flash area
